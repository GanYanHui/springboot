<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-upload :action="'http://' + serverIp + ':9090/image/upload'" :show-file-list="false" :on-success="handleImageUploadSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="type" label="图像类型"></el-table-column>
      <el-table-column prop="presize" label="原图像大小(KB)"></el-table-column>
      <el-table-column prop="nextsize" label="压缩后图像大小(KB)"></el-table-column>
      ...
      <el-table-column label="下载">
        <template slot-scope="scope">
          ...
          <el-button type="warning" @click="preview(scope.row.id)">预览</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    lookImage(id){
      window.open("http://localhost:9090/image/download/" + id)
    },
    preview(id){
      window.open("http://localhost:9090/image/download/" + id)
    },
